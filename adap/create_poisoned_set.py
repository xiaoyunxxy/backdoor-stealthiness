...
if args.poison_type in ['badnet', 'blend', 'none',
    ...
    if args.poison_type == 'badnet':
        ...
        poison_generator = adaptive_blend.poison_generator(img_size=img_size, trainset=train_set, testset=test_set,
                                                          poison_rate=args.poison_rate,
                                                          path=poison_set_img_dir, 
                                                          trigger=trigger, pieces=16, mask_rate=0.5,
                                                          target_class=config.target_class[args.dataset],
                                                          alpha=args.alpha, 
                                                          test_alpha=args.alpha if args.test_alpha is None else args.test_alpha,
    ...
    if args.poison_type not in ['adaptive_blend', 'adaptive_patch', 'adaptive_k_way']:
        ...
        os.mkdir(os.path.join(poison_set_img_dir, "train"))
        os.mkdir(os.path.join(poison_set_img_dir, "test"))
        ...
        poison_generator.generate_poisoned_testing_set()
