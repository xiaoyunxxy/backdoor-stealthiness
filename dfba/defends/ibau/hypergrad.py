...
class DifferentiableOptimizer:
    ...
    def get_loss(self, params, hparams, model, images_list, labels_list):
        if self.data_iterator:
            data = next(self.data_iterator)
            self.curr_loss = self.loss_f(params, hparams, data)
        else:
            self.curr_loss = self.loss_f(params, hparams, model, images_list, labels_list)
        return self.curr_loss

class GradientDescent(DifferentiableOptimizer):
    def __init__(self, loss_f, step_size, model, images_list, labels_list, data_or_iter=None):
    ...
    def step(self, params, hparams, create_graph):
        loss = self.get_loss(params, hparams, self.model, self.images_list, self.labels_list)
...
def fixed_point(params: List[Tensor],
                hparams: List[Tensor],
                model, images_list, labels_list, batchnum,
                K: int ,
    ...
    o_loss = outer_loss(params, hparams,model, images_list, labels_list, batchnum)
